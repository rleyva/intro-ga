...
'''
    Introduction to Genetic Algorithms - Problem #2:
        ...
        f(x) = the base-10 representation of x, where x is a chromosome of length 20.
'''
...
mut_len = 10
pm_array = np.zeros(mut_len, dtype = np.int)
pm_array[random.randint(0,mut_len-1)] = 1
...
class Chromosome:
    ...
    def calculate_fitness(self):
        # This script will call for a small modification to the fitnes function
        # The base-10 representation will dictate the fitness.
        return self.base_10_rep
...
def roulette_wheel_sample():
    ...
    for chromosome in initial_population:
        ...
        if(current >= pick):
            return chromosome
    return initial_population[random.randint(0,99)]
...
def mutate_chromosome(chromosome):
    if pm_array[random.randint(0,mut_len-1)] == 1:
        chromosome.mutate()
